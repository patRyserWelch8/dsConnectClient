...
.test_functions_without_settings <- function(connections)
{
  ...
  expect_equal(dssp.encrypt_param(connections[[1]]),FALSE)
  expect_equal(dssp.decrypt_data(connections[[1]]), FALSE)
  expect_equal(dssp.transfer.coordinates(connections[[1]], connections[[1]]),FALSE)
  expect_equal(dssp.transfer.encrypted.matrix(connections[[1]], connections[[1]], TRUE), FALSE)
  expect_equal(dssp.remove.encryption.data(connections[[1]],TRUE), FALSE)
  expect_equal(dssp.complete.exchange(connections[[1]], param.names), FALSE)
  expect_equal(dssp.exchange(connections[[1]], connections[[1]], param.names), FALSE)
}
...
.test_param <- function(connection)
{
  ...
  expect_equal(as.logical(outcome[[1]][1]),TRUE)
  expect_true(dssp.share.parameter(datasources = connection,param.names = c("pi_value")))
...
.test_no_connection <- function()
{
  ...
  expect_warning(dssp.share.parameter(datasources = connections))
  expect_equal(dssp.assign.settings(connections), FALSE)
  expect_equal(dssp.complete.exchange(connections, param.names), FALSE)
  expect_equal(dssp.exchange(connections, connections, param.names), FALSE)
  expect_equal(dssp.assign.param.settings(connections, param.names), FALSE)
  expect_equal(dssp.encrypt.data(connections,TRUE,FALSE), FALSE)
  expect_equal(dssp.encrypt.param(connections),FALSE)
  expect_equal(dssp.decrypt.data(connections), FALSE)
  expect_equal(dssp.transfer.coordinates(connections, connections), FALSE)
  expect_equal(dssp.transfer.encrypted.matrix(connections, connections, TRUE),FALSE)
  expect_equal(dssp.remove.encryption.data(connections,TRUE), FALSE)
}

.test_single_connection <- function(connection)
{
  expect_equal(length(connection), 1)
  expect_error(dssp.share.parameter(datasources = connection))
  ...
  expect_error(dssp.share.parameter(datasources = connection, param.names = c("pi_value")))
...
.test_multiple_connections <- function(connections)
{
  ...
  #check parameters 
  #expect_warning(dssp.share.parameter(connections))
  ...
  #
  #print(evaluate_promise(dssp.share.parameter(connections,param.names = c('pi_value', 'pi_value_B')),print=TRUE))
