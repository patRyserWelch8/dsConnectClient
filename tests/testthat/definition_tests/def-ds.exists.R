...
.test.variables.does.not.exists<- function(connection,number)
{
  ...
  expect_false(dses.find.variable(datasources = connection,variable.name = name.var,class.type = "integer"))
...
.test.variables.exists<- function(connection,variable.name,value.to.copy,class.type)
{
  ...
  #expect_true(dses.find.variable(datasources = connection,variable.name = variable.name,class.type = class.type))
}

.test.no.connection <- function()
{
  expect_error(dses.find.variable())
  expect_error(dses.find.variable(datasources = NULL,variable.name =  "new.var"))
...
{
  expect_error(dses.find.variable(datasources = connection))
  expect_error(dses.find.variable(datasources = connection, variable.name = NULL))
  expect_error(dses.find.variable(datasources = connection, variable.name = ""))
  expect_error(dses.find.variable(datasources = connection, variable.name = "myVar",class.type = ""))
  expect_error(dses.find.variable(datasources = connection, variable.name = "myVar",class.type = 1))
  expect_error(dses.find.variable(datasources = connection, variable.name = "myVar",class.type = "not a type"))
