...
needs to have installed the following server-side packages: 
...
Note that if you are using windows operating system, the default implementation of SSL used by the curl R package 
has an issue when connecting to a server, using \code{HTTPS}, which has a self-signed certificate. 
The call to \code{Sys.setenv(CURL_SSL_BACKEND = "openssl")} switches implementation of SSL used
by the curl R package to \code{'openssl'}. This will permit login to such DataSHIELD servers.
Thus, before executing any code you need to execute the following in the client-side:
...
The example below uses again the login data obtained in the previous section. Then, the connections object is passed as an argument to obtain all the object listed in the each DataSHIELD server.  
...
The function _ds.assign_value_ enforces a valid R data type being attached to any server variable. No variable is created locally, instead the variable is created and stored on the server. The function also returns an indication whether the assignment process has been achieved on all servers. It is a useful function to use in DataSHIELD scripts to create a variable and verify it has been created on the server.  When developing client-side functions, this function can assist with writing succinct code. The developer can verified the variable has been created with the correct data type.

In the example below, the _server-side function rUnifDs_ generates a uniform distributions and returns a vector _numeric_ values matching the arguments. The returned distribution is saved on the server against a variable named _rUnifDist_. A data type  must match the server-side function outcome. In this case,  it is set to _numeric_, then the outcome is TRUE. Otherwise, no variable is created.
