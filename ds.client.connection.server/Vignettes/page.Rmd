...
The function _ds.assign_value_ enforces a valid R data type being attached to any server variable. No variable is created locally, instead the variable is created on the server. The function also returns an indication whether the assignment process has been achieved on all servers. 
...
```{R}
rUnif.exists <- ds.exists.on.server(connections,
...
# Executing some aggregate DataSHIELD functions
```{R}
server.call <- call("varDS","rUnifDist")
print(server.call)
server.var <- ds.aggregate(connections,expression = server.call, asynchronous = FALSE)

server.call <- call("meanDS","rUnifDist")
print(server.call)
server.mean <- ds.aggregate(connections,expression = server.call, asynchronous = FALSE)
print(paste0("mean     :", server.mean))
print(paste0("variance :", server.var))
